...
class QTable(dict):
    def __init__(self, n_actions, observation_space,float_to_int=False,**kwargs):
        """
        ...
        upper_bounds = [observation_space.high[0],observation_space.high[1],observation_space.high[2],observation_space.high[3]]
        lower_bounds = [observation_space.low[0],observation_space.low[1],observation_space.low[2],observation_space.low[3]]
...
def q_learning(environment: SigmoidMultiValAction,
    ...
    #print('the action space sequence',env.action_space)#Discrete(3)
    #print('number of samples',env.action_space.sample())#0
    #print('meaning of discrete space',env.unwrapped.get_action_meanings())#no function such as get_Action_meanings
    #print('total number of possible states: ',env.observation_space.n)#AttributeError: 'Box' object has no attribute 'n'
    #continous space does not have a finite number of elemnets n
    
    #print('moving up',env.step(1))#this is state,reward,done,info
    print('action space',env.action_space)#Discrete3
    print('observation space',env.observation_space)#box(4,)

    '''
    if the observation space was discrete then we would have done something like,
    ...
    Q = QTable(env.action_space.n, env.observation_space,float_state)  # TODO modify as necessary for your Q-table
